...
%%%
%% core/kazoo_services/src/kz_service_plan.erl:135
-export([list_categories/1]).
%% core/kazoo_services/src/kz_service_plan.erl:136
-export([list_items/2]).
%% core/kazoo_services/src/kz_service_plan.erl:324
-export([quantity/3]).
-export([category_quantity/2
        ,category_quantity/3
        ]).
-export([cascade_quantity/3
        ,cascade_quantities/1
        ]).
-export([cascade_category_quantity/2
        ,cascade_category_quantity/3
        ]).
...
maybe_bookkeeper_allow_updates(AccountId, Status) ->
    Bookkeeper = kz_services_bookkeeper:select_bookkeeper(AccountId),
...
%% @end
%%--------------------------------------------------------------------
-spec quantify(services() | ne_binary()) -> kz_json:object().
quantify(#kz_services{account_id=AccountId}) ->
    quantify(AccountId);
quantify(Account=?NE_BINARY) ->
